import { makeWASocket, useMultiFileAuthState, DisconnectReason, downloadContentFromMessage, Browsers, fetchLatestBaileysVersion } from '@whiskeysockets/baileys';
...
const pendingDebounceMessages = new Map();
const activeSendDelays = new Map(); // jid => { cancel }
...
function queueMessageForDebounce(jid, text, pushName, fromMe, mediaPath, taskMode, isAudio) {
  let pending = pendingDebounceMessages.get(jid);
  if (pending) {
    clearTimeout(pending.timer);
    if (text) {
      pending.text = pending.text ? (pending.text + "\n" + text) : text;
    }
    if (mediaPath && !pending.mediaPaths.includes(mediaPath)) {
      pending.mediaPaths.push(mediaPath);
    }
  } else {
    pending = {
      text: text || '',
  ...
  pending.timer = setTimeout(async () => {
    pendingDebounceMessages.delete(jid);
    console.log(`⏰ [DEBOUNCE] Timer fired for ${jid}. Processing consolidated message.`);

    if (!pending.text.trim() && pending.mediaPaths.length === 0) {
      return;
    }
    ...
    try {
      await axios.post(SERVER_URL, {
        ...
        mediaPath: pending.mediaPaths.length > 0 ? pending.mediaPaths[pending.mediaPaths.length - 1] : null,
        mediaPaths: pending.mediaPaths,
        mode: pending.taskMode
      }, {
        headers: { Authorization: `Bearer ${BRIDGE_SECRET_KEY}` },
      });
      console.log(`✅ Successfully forwarded consolidated message to server (${pending.taskMode}).`);
    } catch (err) {
      console.error("❌ Bridge Forwarding Error (Consolidated):", err.message);
    }
  }, 15000);

  pendingDebounceMessages.set(jid, pending);
}
...
function rememberSelfIdentity(jid = '') {
  const clean = String(jid || '').trim();
  if (!clean) return;
  selfJids.add(clean);
  const digits = normalizeJidUser(clean).replace(/\D/g, '');
  if (digits) selfPhoneIds.add(digits);
}

function isSelfTarget(jid = '') {
  const clean = String(jid || '').trim();
  if (!clean) return false;
  if (selfJids.has(clean)) return true;
  const digits = normalizeJidUser(clean).replace(/\D/g, '');
  return !!digits && selfPhoneIds.has(digits);
}
...
function isTrustedSender(jid, pushName, allowlist) {
  ...
  return !!normalizedPushName && TRUSTED_NAMES.has(normalizedPushName);
}

function isWhitelistedRecipient(jid = '') {
  const clean = String(jid || '').trim();
  if (!clean) return false;
  const whitelist = loadWhitelist();
  const phoneId = normalizeJidUser(clean);
  return Boolean(whitelist[clean] || whitelist[phoneId]);
}
...
async function startWhatsApp() {
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      if (sock?.user?.id) rememberSelfIdentity(sock.user.id);
    }
  });
  ...
  sock.ev.on('messages.upsert', async ({ type, messages }) => {
    ...
    for (const msg of messages) {
      ...
      if (!jid || (!text && !hasMedia)) continue;
      if (fromMe) rememberSelfIdentity(jid);
      ...
      // Download media if it's explicitly triggered OR if it's a trusted conversational chat
      let mediaPath = null;
      if ((isTriggered || trustedSender) && (msg.message?.imageMessage || msg.message?.videoMessage)) {
        try {
          if (msg.message.imageMessage) mediaPath = await downloadMedia(msg.message.imageMessage, 'image', 'jpg');
          else if (msg.message.videoMessage) mediaPath = await downloadMedia(msg.message.videoMessage, 'video', 'mp4');
        } catch (err) {
          console.error(`⚠️ Media download failed: ${err.message}`);
        }
      }

      if (finalTriggered || text || (trustedSender && hasMedia)) {
        ...
        queueMessageForDebounce(jid, text, senderName, fromMe, mediaPath, taskMode, false);
        ...
        if (finalTriggered) {
          try {
            ...
            console.error('❌ Failed to send working status:', err.message);
          }
        }
      }
    }
...
app.post('/send', async (req, res) => {
  ...
  // Final send-time authorization:
  // - command authorization is handled on inbound task routing
  // - conversational outbound is allowed only to self or explicit whitelist recipients
  if (!isSelfTarget(jid) && !isWhitelistedRecipient(jid)) {
    return res.status(403).json({ error: 'Recipient not authorized' });
  }

  try {
    const isSystemMsg = /^[✅⏳❌🤖♊]/.test(text) || text.includes('Gemini CLI') || text.includes('Codex CLI') || text.includes('GeminiCLI');
    if (!isSystemMsg) {
      const existingDelay = activeSendDelays.get(jid);
      if (existingDelay) {
        console.log(`🧹 [SEND] Found active pending send for ${jid}. Cancelling it to avoid multiple replies.`);
      ...
      const delayPromise = new Promise((resolve, reject) => {
        currentTimer = setTimeout(resolve, 30000);
        activeSendDelays.set(jid, {
          cancel: () => {
            isCancelled = true;
      ...
      console.log(`⏳ Waiting 30s (silence) before replying to ${jid}...`);
      try {
        await delayPromise;
      } catch (err) {
        if (isCancelled) {
          console.log(`🚫 [SEND] Discarded response because a newer message arrived: "${text.substring(0, 40)}..."`);
          return res.json({ status: 'cancelled', message: 'superseded by newer message' });
        }
        throw err;
      }

      const composingPromise = new Promise((resolve, reject) => {
        currentTimer = setTimeout(resolve, 10000);
        activeSendDelays.set(jid, {
          cancel: () => {
            isCancelled = true;
            clearTimeout(currentTimer);
      ...
      console.log(`⏳ Simulating typing for 10s to ${jid}...`);
      try {
        await global.whatsappSock.sendPresenceUpdate('composing', jid);
        await composingPromise;
        await global.whatsappSock.sendPresenceUpdate('paused', jid);
      } catch (err) {
        try { await global.whatsappSock.sendPresenceUpdate('paused', jid); } catch {}
        if (isCancelled) {
          console.log(`🚫 [SEND] Discarded response during typing: "${text.substring(0, 40)}..."`);
          return res.json({ status: 'cancelled', message: 'superseded by newer message' });
        }
        throw err;
      }

      activeSendDelays.delete(jid);
    }
